<template>
    <q-page>
      <Titulo
        titulo="Unidades Descentralizadas"
        icono="account_tree"
      ></Titulo>
      <CrudTable
        :filters="filters"
      ...
      >
        <template v-slot:buttons="{ open }">
          <q-btn
            ...
            label="Nueva Unidad Descentralizada"
          />
        </template>
        <template v-slot:form="{ close, update}">
          <q-card style="width: 700px; max-width: 90vw;">
            <q-toolbar class="q-pa-md">
              ...
              />
              <q-toolbar-title>
                {{ menu.id ? 'Editar' : 'Agregar' }} Unidad Descentralizada
              </q-toolbar-title>
              <q-space />
              <q-btn
                flat
...
<q-td>
  ...
  import Menu from 'components/Formularios/Descentralizadas.vue'
  ...
  export default {
    ...
    setup () {
      ...
      const url = ref('/descentralizadas/')
      const menu = ref({
        grupo: null,
        nombre: null,
        descripcion: null,
      ...
      const guardar = async (update, close) => {
        const mensaje = !menu.value.id ? 'Unidad creado de manera exitosa.' : 'Unidad actualizado de manera exitosa.'
        if (menu.value.id) {
