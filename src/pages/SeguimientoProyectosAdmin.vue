<template>
  <q-page>
    <Titulo titulo="Pendientes" icono="fact_check"></Titulo>
    <CrudTable
      :filters="filters"
      :columns="columns"
      :url="url"
      ...
      <template v-slot:row="{ row, open }">
        ...
        >
          <q-td style="text-align: center"
            ><span class="text-weight-bold">{{ row.sector }}</span></q-td
          >
...
export default {
  ...
  setup() {
    ...
    const url = ref("/seguimientos-admin/");
    const menu = ref({
      grupo: 1,
      nombre: null,
      descripcion: null,
    ...
    onMounted(async () => {
      filters[2].options = (await _http.get("/sectores/")).results.map((r) => ({
        label: r.nombre,
        value: r.id,
      }));
    });
