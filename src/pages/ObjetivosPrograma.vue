<template>
    <q-page>
      <Titulo titulo="Objetivo-Programas" icono="assignment_turned_in"></Titulo>
        ...
        <template v-slot:form="{ close, update }">
          <q-card style="width: 1200px; max-width: 90vw">
            <q-toolbar class="q-pa-md">
              ...
              <q-toolbar-title>
                {{ proyecto.id ? "Editar" : "Agregar" }} Objetivo
              </q-toolbar-title>
              <q-space />
          ...
          <q-tr class="q-tr-table">
            ...
            <q-td>
              <q-btn
                ...
                round
                icon="edit"
                @click="openModal(open, row.objetivo)"
              />
              <q-btn
                class="q-pa-xs"
                flat
                ...
                color="negative"
                icon="delete"
                @click="eliminar({ url: `${url}${row.objetivo}/` })"
              />
  ...
  import ObjetivosForm from "components/Formularios/ObjetivosForm.vue";
  ...
  export default {
    components: { CrudTable, ObjetivosForm, FichaProyecto },
    name: "Programas",
    setup() {
      const route = useRoute();
      const $q = useQuasar();
      const _http = inject("http");
     const proId =ref(route.params.programa_id);
    ...
    console.log("holaaaaaaaaaaa", proId);
      ...
      const url = ref("/programas/"+proId.value+"/objetivos/");
      const proyecto = ref({
        tipo_migrado: "nuevo",
      });
      ...
      const guardar = async (update, close) => {
        ...
        if (proyecto.value.id) {
          await _http.put(`${url.value}${proyecto.value.objetivo}/`, proyecto.value);
        } else {
          await _http.put(`${url.value}${proyecto.value.objetivo}/`, proyecto.value);
        }
