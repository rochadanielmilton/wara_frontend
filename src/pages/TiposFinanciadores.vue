<template>
        ...
        <template v-slot:form="{ close, update}">
          <q-card style="width: 700px; max-width: 90vw;">
            <q-toolbar class="q-pa-md">
              ...
              />
              <q-toolbar-title>
                {{ menu.id ? 'Editar' : 'Agregar' }} Tipo - Financiador
              </q-toolbar-title>
              <q-space />
              <q-btn
                flat
  ...
  import Menu from 'components/Formularios/FormFinanciadores.vue'
  ...
  export default {
    ...
    setup () {
      ...
      const url = ref('/tipos-financiamiento/')
      const menu = ref({
        grupo: null,
        nombre: null,
        descripcion: null,
      ...
      const guardar = async (update, close) => {
        const mensaje = !menu.value.id ? 'Departamento creado de manera exitosa.' : 'Departamento actualizado de manera exitosa.'
        if (menu.value.id) {
