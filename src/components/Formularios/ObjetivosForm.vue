<template>
    <q-form ref="usuario" @submit="$emit('guardar')" class="row q-col-gutter-md">
      <q-input square class="col-xs-12 col-md-6" label="Valor" v-model="valoresModel.valor"
        ...
        <q-select v-model="valoresModel.objetivo" square class="col-xs-12 col-md-6" :options="roles"
        label="Objetivo" option-value="id" option-label="titulo" use-chips emit-value map-options
  ...
  export default {
    ...
    setup(props) {
      ...
      async function getDepartamentos() {
        try {
          const {results} = await _http.get("/objetivos/");
          return results;
        } catch (error) {
